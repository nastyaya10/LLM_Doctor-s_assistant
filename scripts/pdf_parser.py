...
# 1. Убеждаемся, что Python видит корень проекта
PROJECT_ROOT = Path(__file__).resolve().parents[1]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

# 2. Импортируем пути из нашего централизованного модуля
from src.services.paths import RAW_DATA, PARSED_DATA

# 3. Назначаем пути согласно новой структуре
DEFAULT_INPUT = RAW_DATA
DEFAULT_OUTPUT_DIR = PARSED_DATA
